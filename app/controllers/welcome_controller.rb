...
class WelcomeController < ApplicationController
  def index
    ...
    @orders_my_warehouse = doc.xpath('//collection/customerOrder').map do |i|
      {'uuid' => i.xpath('uuid').text(),'groupUuid' => i.xpath('groupUuid').text(), 'customerOrderPosition' => {'quantity' => i.xpath('customerOrderPosition/@quantity').text(),
    ...
    unless @orders_my_warehouse.blank?
      @orders_my_warehouse.each do |order_my_warehouse|
        if Order.where(:uuid => order_my_warehouse["uuid"]).blank?
          Order.create(:uuid => order_my_warehouse["uuid"],:good_id => order_my_warehouse['customerOrderPosition']['goodUuid'],
                       :agent_id => order_my_warehouse['customerOrderPosition']['sourceAgentUuid'], :quantity => order_my_warehouse['customerOrderPosition']['quantity'])
        else
          query = Order.find_by(:uuid => order_my_warehouse["uuid"])
          query.update(:good_id => order_my_warehouse['customerOrderPosition']['goodUuid'], :agent_id => order_my_warehouse['customerOrderPosition']['sourceAgentUuid'],
