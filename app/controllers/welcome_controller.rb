...
class WelcomeController < ApplicationController
  ...
  def index
    # response = moy_sklad_get('CustomerOrder',"list?start=0&count=1000", @@company)
    # doc = Nokogiri::XML(response.body)
    # total = doc.xpath('//collection/@total').text().to_i
    # render json: fill_orders(doc)
    # if total > 1000
    #   counter = 1
    #   count_response = total/1000
    #   while count_response >= counter
    #     response = moy_sklad_get('CustomerOrder',"list?start=#{counter*1000}&count=1000", @@company)
    #     doc = Nokogiri::XML(response.body)
    #     fill_orders(doc)
    #     counter+=1
    #   end
  ...
  def upload_latest_orders
    time = Date.today
    time_day_ago = time - 1
    response = moy_sklad_get('CustomerOrder',"#{URI.encode("list?start=0&count=1000&filter=updated>#{time_day_ago.year}#{time_day_ago.strftime('%m')}#{time_day_ago.strftime('%d')}010000")}", @@company)
    doc = Nokogiri::XML(response.body)
    total = doc.xpath('//collection/@total').text().to_i
    fill_orders(doc)
    if total > 1000
      ...
      count_response = total/1000
      while count_response >= counter
        response = moy_sklad_get('CustomerOrder',"#{URI.encode("list?start=#{counter*1000}&count=1000&filter=updated>#{time_day_ago.year}#{time_day_ago.strftime('%m')}#{time_day_ago.strftime('%d')}010000")}", @company)
        doc = Nokogiri::XML(response.body)
        fill_orders(doc)
        counter+=1
      end
  ...
  def delete_orders(orders_my_warehouse)
    @orders = Order.all
  ...
  def moy_sklad_get(entityType, uuid, company)
    moy_sklad_request(entityType, uuid, nil, :get, company["moy_sklad_login"], company["moy_sklad_password"])
  end

  def moy_sklad_request(entityType, entityId, xml, request_type, login, password)
